...
void FileManager::Rdwr( enum File::FileFlags mode )
{
	...
	这不是V6的设计。read、write系统调用对内存i节点上锁是为了给实施IO的进程提供一致的文件视图。*/
	{
		...
		Diagnose::Write("u.u_IOParam.m_Offset: %d\n",u.u_IOParam.m_Offset);
		if ( File::FREAD == mode )
		{
			pFile->f_inode->ReadI();
		}
...
Inode* FileManager::NameI( char (*func)(), enum DirectorySearchMode mode )
{
	...
	while (true)
	{
		...
		{
			if ( pChar < &(u.u_dbuf[DiskDirectoryEntry::ADD_DIRSIZ + DiskDirectoryEntry::DIRSIZ]) )
			{
				*pChar = curchar;
				pChar++;
			}
		...
		/* 将u_dbuf剩余的部分填充为'\0' */
		while ( pChar < &(u.u_dbuf[DiskDirectoryEntry::ADD_DIRSIZ + DiskDirectoryEntry::DIRSIZ]))
		{
			*pChar = '\0';
			pChar++;
		}
		Diagnose::Write("u.u_dbuf: %s\n",u.u_dbuf);
		...
		/* 设置为目录项个数 ，含空白的目录项*/
		u.u_IOParam.m_Count = pInode->i_size / (DiskDirectoryEntry::DIRSIZ + 8);
		freeEntryOffset = 0;
		pBuf = NULL;
		Diagnose::Write("u.u_IOParam.m_Count: %d\n",u.u_IOParam.m_Count);
		...
		while (true)
		{
			...
			if ( 0 == u.u_IOParam.m_Count )
			{
				...
				if ( FileManager::CREATE == mode && curchar == '\0' )
				{
					...
					if ( freeEntryOffset )	/* 此变量存放了空闲目录项位于目录文件中的偏移量 */
					{
						...
						u.u_IOParam.m_Offset = freeEntryOffset - (DiskDirectoryEntry::DIRSIZ + 8);
					}
			...
			Utility::DWordCopy( src, (int *)&u.u_dent, sizeof(DiskDirectoryEntry)/sizeof(int) );
			Diagnose::Write("u.u_dent: inode:%d, name:%s, add:%d\n",u.u_dent.m_ino,u.u_dent.m_name,u.u_dent.m_addition_esize);

			u.u_IOParam.m_Offset += (DiskDirectoryEntry::DIRSIZ + 8);
			u.u_IOParam.m_Count--;
			...
			if ( 0 == u.u_dent.m_ino )
			{
				...
				{
					int length=Utility::StringLength(u.u_dirp);
					int use_entries=(length-DiskDirectoryEntry::DIRSIZ+sizeof(DiskDirectoryEntry)-1)/sizeof(DiskDirectoryEntry);
					if(use_entries<=u.u_dent.m_addition_esize)
					{
						...
						if(use_entries<=u.u_dent.m_addition_esize)
						{
							...
							{
								/* 计算要读的物理盘块号 */
								int phyBlkno = pInode->Bmap((use_entries*sizeof(DiskDirectoryEntry)+u.u_IOParam.m_Offset) / Inode::BLOCK_SIZE );
								Buf* newBuf = bufMgr.Bread(pInode->i_dev, phyBlkno );
								int* src = (int *)(newBuf->b_addr + ((use_entries*sizeof(DiskDirectoryEntry)+u.u_IOParam.m_Offset) % Inode::BLOCK_SIZE));
								DiskDirectoryEntry newEntry;
								newEntry.m_addition_esize=u.u_dent.m_addition_esize-use_entries-1;
								Utility::DWordCopy( src, (int *)&newEntry, sizeof(DiskDirectoryEntry)/sizeof(int) );
								if ( NULL != newBuf )
								{
									bufMgr.Brelse(newBuf);
								}
							}
						}
					}
				...
				/* 跳过空闲目录项，继续比较下一目录项 */
				u.u_IOParam.m_Offset += (DiskDirectoryEntry::DIRSIZ + 8) * u.u_dent.m_addition_esize;
				continue;
			}

			/* 有附加块，继续向下读取*/
			if(u.u_dent.m_addition_esize > 0)
			{
				int addition_index=1;
				while(addition_index <= u.u_dent.m_addition_esize)
				{
					/* 已读完目录文件的当前盘块，需要读入下一目录项数据盘块 */
					if ( 0 == u.u_IOParam.m_Offset % Inode::BLOCK_SIZE )
					{
					...
					int* src = (int *)(pBuf->b_addr + (u.u_IOParam.m_Offset % Inode::BLOCK_SIZE));
					Utility::DWordCopy( src, (int *)&u.addition_entry[(addition_index-1)*sizeof(DiskDirectoryEntry)], sizeof(DiskDirectoryEntry)/sizeof(int) );

					u.u_IOParam.m_Offset += sizeof(DiskDirectoryEntry);
					u.u_IOParam.m_Count--;
					addition_index++;
				}
			}
			Diagnose::Write("u.addition_entry:%s\n",u.addition_entry);


			int i;
			for ( i = 0; i < DiskDirectoryEntry::DIRSIZ + DiskDirectoryEntry::ADD_DIRSIZ; i++ )
			{
				if(i < DiskDirectoryEntry::DIRSIZ)
				{
				...
				else if(i < DiskDirectoryEntry::DIRSIZ + u.u_dent.m_addition_esize * sizeof(DiskDirectoryEntry))
				{
					if ( u.u_dbuf[i] != u.addition_entry[i-DiskDirectoryEntry::DIRSIZ] )
					{
						break;	/* 匹配至某一字符不符，跳出for循环 */
					}
				}
			...
			if( i < DiskDirectoryEntry::DIRSIZ + u.u_dent.m_addition_esize * sizeof(DiskDirectoryEntry))
			{
				/* 不是要搜索的目录项，继续匹配下一目录项 */
				continue;
			}
...
void FileManager::WriteDir( Inode* pInode )
{
	...
	/* 设置目录项中pathname分量部分 */
//	for ( int i = 0; i < DiskDirectoryEntry::DIRSIZ; i++ )
//	{
//		u.u_dent.m_name[i] = u.u_dbuf[i];
//	}
	u.u_dent.m_addition_esize = (Utility::StringLength(u.u_dbuf) - DiskDirectoryEntry::DIRSIZ + sizeof(DiskDirectoryEntry)-1) / sizeof(DiskDirectoryEntry);
	int i;
	for ( i = 0; i < DiskDirectoryEntry::DIRSIZ + DiskDirectoryEntry::ADD_DIRSIZ; i++ )
	{
		if(i < DiskDirectoryEntry::DIRSIZ)
		...
		{
			u.addition_entry[i-DiskDirectoryEntry::DIRSIZ] = u.u_dbuf[i];
		}
	}

	Diagnose::Write("distance of u.addition_entry-u.u_dent: %d==32?\n",(int)u.addition_entry-(int)&u.u_dent);
	u.u_IOParam.m_Count = DiskDirectoryEntry::DIRSIZ + 8 + u.u_dent.m_addition_esize * sizeof(DiskDirectoryEntry);
	u.u_IOParam.m_Base = (unsigned char *)&u.u_dent;
	u.u_segflg = 1;
	Diagnose::Write("WriteDir::u.u_dent::m_name: %s, m_addition_esize: %d\n",u.u_dent.m_name,u.u_dent.m_addition_esize);
	Diagnose::Write("WriteDir::u.addition_entry: %s\n",u.addition_entry);
...
void FileManager::UnLink()
{
	...
	/* 写入清零后的目录项 */
	u.u_IOParam.m_Offset -= (DiskDirectoryEntry::DIRSIZ + 8 + u.u_dent.m_addition_esize * sizeof(DiskDirectoryEntry));
	u.u_IOParam.m_Base = (unsigned char *)&u.u_dent;
	u.u_IOParam.m_Count = DiskDirectoryEntry::DIRSIZ + 8 + u.u_dent.m_addition_esize * sizeof(DiskDirectoryEntry);
	
	for (int i = 0; i < DiskDirectoryEntry::ADD_DIRSIZ; i++ )
	{
		u.addition_entry[i] = '\0';
	}
	u.u_dent.m_ino = 0;
	//u.u_dent.m_addition_esize = 0;	此项保留
	Diagnose::Write("u.u_IOParam.m_Offset: %d\n",u.u_IOParam.m_Offset);
	Diagnose::Write("UnLink::u.u_dent::m_name: %s, m_addition_esize: %d\n",u.u_dent.m_name,u.u_dent.m_addition_esize);
	Diagnose::Write("UnLink::u.addition_entry: %s\n",u.addition_entry);
	pDeleteInode->WriteI();
...
DiskDirectoryEntry::DiskDirectoryEntry()
{
	this->m_ino = 0;
	this->m_name[0] = '\0';
	this->m_addition_esize = 0;
}
