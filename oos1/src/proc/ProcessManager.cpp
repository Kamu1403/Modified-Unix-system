...
//	if ( NULL != pgTable ) /*针对0#进程没有虚实地址映射表*/
//	{
//		//u.u_MemoryDescriptor.Initialize();
...
loop:
	...
	for ( int i = 0; i < ProcessManager::NPROC; i++ )
	{
		if ( this->process[i].p_flag & (Process::SSYS | Process::SLOCK | Process::SLOAD) == Process::SLOAD && (this->process[i].p_stat == Process::SRUN || this->process[i].p_stat == Process::SSLEEP) && pSelected->p_time > seconds )
		{
			pSelected = &(this->process[i]);
			seconds = pSelected->p_time;
		}
...
//	}
	...
	/* 清0所有通用寄存器，不可以清 EBP  */
	unsigned char * p;
	p = (unsigned char *)u.u_ar0 - 4*6;

	for (int i = 0; i < 5 ; i++)
	{
		*((int *) p ) = 0;
		// Diagnose::Write("p == %x; i == %d;\n", p,i);
		p += 4;
	}
	/*    这段代码也是对的。可读性强，但效率低一点。
	for (int i = User::EAX - 6; i < User::EAX - 1 ; i++)
	{
    	u.u_ar0[i] = 0;
    	Diagnose::Write("u_ar0[%d]== %d\n", i,u.u_ar0[i]);
	}*/
//	/* 清0所有通用寄存器  */
